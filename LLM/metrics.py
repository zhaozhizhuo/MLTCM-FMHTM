...
def all_metrics(yhat, y, k=[1, 2, 3], yhat_raw=None, calc_auc=True):
    """
        Inputs:
...
def ACCuRACY(yhatmic, ymic):
    xx = intersect_size(yhatmic, ymic, 0)
    print(xx)
    x = intersect_size(yhatmic, ymic, 0).sum(axis=0)
    x = x/len(yhatmic)
    return x
