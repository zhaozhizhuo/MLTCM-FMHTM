...
def dataload(embedding_weights, word_to_idx, idx_to_word,path):
    # 找到所有的标签
    syndromes = []
    with open('../data_preprocess/cardiovascular_diag.json', 'r', encoding='utf-8') as file:
        for line in file:
...
datasets = ['train','dev','test']
for dataset in datasets:
    path = '../data_preprocess/{}.json'.format(dataset)
...
class BILSTM(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.LSTM(100, 512, num_layers=2,
        ...
        # laat
        self.third_linears_laat = nn.Linear(1024, args.n_labels, bias=True)
        self.first_linears = nn.Linear(1024, 256, bias=False)
        self.second_linears = nn.Linear(256, args.n_labels, bias=False)
    ...
    def forward(self,batch_data):
        ...
        lengths = torch.tensor([512] * batch_size)
